...
[+] Endpoint GET /api/admin/orders para listar pedidos de administradores
[+] Endpoint GET /api/admin/decode-token para debug de tokens JWT
*/
...
// ✅ CAMBIO 3: Endpoint para listar pedidos (orders) - agregado exactamente como solicitado
app.get('/api/admin/orders', verifyAdmin, async (req, res) => {
try {
const { data: orders, error } = await supabase
.from('orders')
.select('*, user:buyer_id(email, full_name)')
.order('created_at', { ascending: false });
    if (error) throw error;
...
// 🐛 DEBUG: Decodificar Token (Público para testing)
app.get('/api/admin/decode-token', async (req, res) => {
    ...
    try {
        ...
        const decoded = jwt.verify(token, process.env.JWT_SECRET || 'reshop-secret-key-2026');
